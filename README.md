...
`Check NJFU Grades` 根据频率预设定时执行，也可以手动运行。定时任务在 `MONITOR_ENABLED=false`、未到开始日期或超过停止日期时，会在安装依赖和登录之前结束，并在 Actions 摘要中显示跳过原因。手动运行默认勾选 `force_check`，会忽略监控开关和日期范围，实际查询一次；取消勾选时才按定时任务的边界检查。达到完成条件后只发送一次完成提醒，仍会低频检查到停止日期，以捕捉新增或更正成绩。
...
如果停止日期后又公布了成绩，直接手动运行 `Check NJFU Grades`，保留默认的 `force_check`，不要勾选 `reset_state`。这样会保留上次状态，只通知真正新增或更正的成绩。`reset_state` 仅用于状态损坏或准备重新建立基线；默认不会把重置时已经存在的成绩重新通知。
